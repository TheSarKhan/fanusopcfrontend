...
const whatsappSupportHref = (msg: string) =>
  "https://wa.me/994993585888?text=" + encodeURIComponent(msg);
...
function ModuleLock({ enabled, children }: { enabled: Set<string> | null; children: React.ReactNode }) {
  ...
  return (
    <div style={{ maxWidth: 520, margin: "48px auto", background: "#fff", border: "1px solid var(--brand-100, #E3ECF9)", borderRadius: 16, padding: 32, textAlign: "center" }}>
      ...
      <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--oxford)", margin: "0 0 8px" }}>{t("psyLock.title")}</h2>
      <p style={{ fontSize: 14, color: "var(--oxford-60)", lineHeight: 1.6, margin: "0 0 20px" }}>
        {t("psyLock.body")}
      </p>
      <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
        <a href={whatsappSupportHref(t("psyLock.whatsappMsg"))} target="_blank" rel="noopener noreferrer" className="fx-btn fx-btn--primary" style={{ textDecoration: "none" }}>{t("psyLock.contactSupport")}</a>
        <Link href="/psycholog" className="fx-btn fx-btn--ghost" style={{ textDecoration: "none" }}>{t("psyLock.backHome")}</Link>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fx-overlay fx-overlay--center" onClick={onClose} style={{ zIndex: 9000, padding: 20 }}>
      <div className="fx-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 420, textAlign: "center" }}>
        ...
        <h3 className="fx-h3">{t("psyLock.modalTitle", { label })}</h3>
        <div className="fx-modal__text">
          {t("psyLock.body")}
        </div>
        <div className="fx-modal__actions">
          <button type="button" onClick={onClose} className="fx-btn fx-btn--ghost">{t("psyLock.close")}</button>
          <a href={whatsappSupportHref(t("psyLock.whatsappMsg"))} target="_blank" rel="noopener noreferrer" className="fx-btn fx-btn--primary" style={{ textDecoration: "none" }}>{t("psyLock.contactSupport")}</a>
        </div>
      </div>
    </div>
