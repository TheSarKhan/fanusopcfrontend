    ...
    <div>
      <div className="mb-6 flex items-end justify-between">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          style={{ padding: "8px 14px", border: "1px solid #E5E7EB", borderRadius: 10, fontSize: 13, width: 280 }} />
      </div>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {filtered.map(c => (
            <Link key={c.patientId} href={`/psycholog/clients/${c.patientId}`}
              ...
              <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#EEF2FF", color: "#3730A3", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14 }}>
              ...
              <div style={{ display: "flex", gap: 16, fontSize: 12, color: "#52718F" }}>
                <div><strong style={{ color: "#1A2535" }}>{c.totalSessions}</strong> seans</div>
                <div><strong style={{ color: c.noteCount > 0 ? "#5A4FC8" : "#1A2535" }}>{c.noteCount}</strong> qeyd</div>
              </div>
