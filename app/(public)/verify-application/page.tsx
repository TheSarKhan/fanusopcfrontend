...
import { verifyApplicationEmail } from "@/lib/api";
import { useT } from "@/lib/i18n/LocaleProvider";

/** Psixoloq müraciəti üçün email təsdiqi (V150) — /verify-dən fərqli: burada
 *  hələ heç bir hesab yoxdur (yalnız PsychologistApplication), ona görə uğur
 *  ekranı "daxil ol"a yox, ana səhifəyə yönləndirir. */
      ...
      <div style={{
        ...
        {status === "loading" && (
          <>
            ...
            <p className="text-[#52718F] text-sm mt-2">{t("authPage.verifyAppCheckingSub")}</p>
          </>
        )}
        ...
        {status === "success" && (
          <>
            ...
            <h2 className="text-xl font-bold text-[#1A2535] mb-2">{t("authPage.verifyAppVerifiedTitle")}</h2>
            <p className="text-[#52718F] text-sm leading-relaxed mb-6">
              {t("authPage.verifyAppVerifiedBody")}
            </p>
            <Link
              href="/"
        ...
        {status === "error" && (
          <>
            ...
            <h2 className="text-xl font-bold text-[#1A2535] mb-2">{t("authPage.verifyAppUsedTitle")}</h2>
            <p className="text-[#52718F] text-sm leading-relaxed mb-6">
              {message || t("authPage.verifyAppUsedBody")}
            </p>
            <Link
              href="/register"
              className="block py-3 rounded-xl text-sm font-bold"
